...
def save_symbolic_expression(kernels, biases, fns_list, save_path, round_decimals):
    """
    Saves a symbolic expression of network as pngs and TXT.
    """
    ...
    for i, result in enumerate(res):
        # Redondeo
        result = round_sympy_expr(result, round_decimals) # Nota: Corregí una pequeña asignación aquí respecto al original
        
        # Simplificación
        try:
            result = proper_simplify(result) # Nota: Asignar el resultado simplificado a la variable result
        ...
        # --- BLOQUE NUEVO: GUARDAR COMO TEXTO ---
        # Guardar formato legible por Python/Humano
        with open(path.join(save_path, 'formula_y' + str(i) + '.txt'), 'w') as f:
            f.write(str(result))
            
        # Opcional: Guardar también el código LaTeX en texto
        with open(path.join(save_path, 'formula_y' + str(i) + '_latex.txt'), 'w') as f:
            f.write(sympy.latex(result))
        # ----------------------------------------

        # Código original de imágenes
        expr_to_latex_png(result, path.join(save_path, 'latex_y' + str(i) + '.png'))
